...
def saving_increment(label,name,img):
    ...
    cv2.imwrite(f'images/{label}/{name}', img)
    st.session_state.counter += 1

def saving_increment_crop(label,name,img):
    ## Incrementa o contador para obter a próxima foto
    st.session_state.image_labels.append(label)
    img = img.save(f'images/{label}/{name}')
    ...
    if uploaded_files and labels:
        ...
        if st.session_state.crop_button:
            try:
                ...
                # preview = st_cropper(img, realtime_update=realtime_update, box_color='yellow')
                ...
                # my_bar = st.progress((st.session_state.counter/len(images)), text=progress_text)
            ...
            try:
                image_data = cv2.imdecode(np.frombuffer(images[st.session_state.counter].read(), np.uint8), 1)
                image_print = cv2.cvtColor(np.array(image_data), cv2.COLOR_BGR2RGB)
                st.image(image_print, caption=f"Image {st.session_state.counter+1}")

                st.subheader('Choose a label for the image')
                label = st.radio(f"label:",[labels[i] for i in range(len(labels))])

                st.button(f"Next image",on_click=saving_increment,args=[label,images[st.session_state.counter].name,image_data])
                st.progress((st.session_state.counter/len(images)), text=progress_text)
